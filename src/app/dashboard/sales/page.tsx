        ...
        <div className="space-y-6">
                ...
                {[
                    ...
                    <Card key={i} className="border-none shadow-sm transition-transform hover:scale-[1.02] cursor-pointer" onClick={() => toast.info(`Viewing trends for ${stat.title}`)}>
                        ...
                        <CardContent>
                            ...
                            <p className={`text-[10px] font-bold flex items-center gap-1 mt-1 ${stat.pos ? 'text-emerald-500' : 'text-rose-500'}`}>
            ...
            <Card className="border-none shadow-md">
                <CardHeader className="px-6 py-4">
                    ...
                    <CardDescription>A list of your most recent sales across all channels.</CardDescription>
                </CardHeader>
                                ...
                                {sales.map((sale) => (
                                    <TableRow key={sale.id} className="hover:bg-muted/10 transition-colors border-b-muted/50 cursor-pointer text-start" onClick={() => toast.info(`Transaction #${sale.id} details`)}>
                                        ...
                                        <TableCell className="text-right px-6 py-4">
                                            <Badge className={`font-bold uppercase tracking-wider text-[9px] border-none px-2.5 py-0.5 ${sale.status === 'Paid' ? 'bg-emerald-500 text-white shadow-sm' :
                                                sale.status === 'Pending' ? 'bg-amber-500 text-white shadow-sm' :
                                                    'bg-rose-500 text-white shadow-sm'
                                                }`}>
                                                {sale.status}
                                            </Badge>
                                        </TableCell>
                                        <TableCell className="text-right font-bold text-sm px-6 py-4">{sale.amount}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
